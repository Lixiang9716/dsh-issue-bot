...
const DEFAULTS = {
  ...
  // 模型路由:全空 = 跟随 GUI 默认选择(agentDefaultModel)
...
export function apply(ctx) {
  ...
  async function loadConfigFromDisk() {
    try {
      ...
      // 文件损坏(而非不存在):先备份再写种子,绝不静默覆盖用户的配置
      try {
        await rename(CONFIG_PATH, CONFIG_PATH + '.invalid-' + Date.now())
        console.error('[issue-bot] config file unreadable, backed up before reseeding: ' + String(error))
      } catch {
        // ENOENT:首次启动尚无配置文件,正常
      }
      await persistConfig().catch(() => {})
    }
  }

  async function persistConfigOf(config) {
  ...
  async function persistConfig() {
    await persistConfigOf(state.config)
  }
  ...
  async function handleIssue(entry, issue) {
    ...
    if (modelCfg.provider !== '' && modelCfg.model !== '') {
      ...
      if (selection !== null && selection !== undefined
        ...
        effort = typeof selection.reasoningEffort === 'string' ? selection.reasoningEffort : ''
      }
    }
    if (provider === undefined || modelId === undefined) {
      throw new Error('no usable model route (config empty and agentDefaultModel missing or empty); cannot start issue agent')
    }
    const agentOptions = { provider, model: modelId }
    entry.model = provider + '/' + modelId + (effort !== '' ? ' · effort ' + effort : '')
    let presetId
    if (presets !== undefined) presetId = (await presets.resolve()).id
    const sessionId = 'session-' + randomUUID()
    entry.sessionId = sessionId
    const createOptions = { sessionId, agentOptions, meta: { cwd } }
    const applySetup = async (agentCtx) => {
      if (presets !== undefined) await presets.mount(agentCtx, presetId)
      // 思考强度经 agent 作用域的 agent/request 瀑布注入(仅影响本 agent)
      if (effort !== '') {
    ...
    createOptions.setup = applySetup
  ...
  // ===== 模型目录(供配置 UI 下拉;60 秒缓存) =====
  let modelCatalogCache = null
  let modelCatalogAt = 0

  async function modelCatalog() {
    if (modelCatalogCache !== null && Date.now() - modelCatalogAt < 60000) return modelCatalogCache
    const llm = ctx.get('llm')
    if (llm === undefined) {
      return { available: false, providers: [], defaultSelection: null }
    }
    const providers = []
    for (const info of llm.listProviders()) {
      ...
      let listed = []
      try {
        listed = await llm.listModels(info.id)
      } catch (error) {
        listed = []
      }
      for (const model of listed) {
        const entryModel = { id: model.id, name: model.name, efforts: [], defaultEffort: null }
        try {
          const resolved = await llm.resolveModelInfo(info.id, model.id)
          if (resolved !== null && resolved !== undefined && resolved.reasoning !== undefined) {
            entryModel.efforts = (resolved.reasoning.efforts ?? []).map((item) => ({ id: item.id, name: item.name }))
            entryModel.defaultEffort = resolved.reasoning.defaultEffort ?? null
          }
        } catch (error) {
          // 该模型不支持解析出思考强度:保持空列表
        }
      ...
      providers.push({ id: info.id, name: info.name, models })
    }
    const modelService = ctx.get('agentDefaultModel')
    ...
    const catalog = { available: true, providers, defaultSelection }
  ...
  async function setConfig(patch) {
    ...
    const next = {
      ...
      model: sanitizeModel(state.config.model),
    }
    try {
      ...
      if (patch.model !== undefined) {
        ...
        const bothSet = next.model.provider !== '' && next.model.model !== ''
        const bothEmpty = next.model.provider === '' && next.model.model === ''
        if (!bothSet && !bothEmpty) throw new Error('模型 provider 与 model 需同时设置,或同时留空(跟随默认)')
      }
    } catch (error) {
    ...
    // 模型路由可用性:provider/model 需能解析;思考强度需在该模型支持的列表内
    if (next.model.provider !== '' && next.model.model !== '') {
      const llm = ctx.get('llm')
      if (llm === undefined) {
        failures.push('llm 服务不可用,无法验证模型路由')
      } else {
        try {
          const resolved = await llm.resolveModelInfo(next.model.provider, next.model.model)
          if (next.model.reasoningEffort !== '') {
            const efforts = resolved !== null && resolved !== undefined && resolved.reasoning !== undefined
              ? (resolved.reasoning.efforts ?? []).map((item) => item.id)
              : []
            if (!efforts.includes(next.model.reasoningEffort)) {
              failures.push('模型 ' + next.model.provider + '/' + next.model.model + ' 不支持思考强度 "' + next.model.reasoningEffort + '"(可选:' + (efforts.length > 0 ? efforts.join(', ') : '无') + ')')
            }
          }
        } catch (error) {
          failures.push('模型路由不可用 ' + next.model.provider + '/' + next.model.model + ':' + String(error && error.message ? error.message : error))
        }
      }
    }
    ...
    // 落盘先行:磁盘写入成功后才切换内存并热生效;写盘失败时原配置原样保留
    const nextConfig = {
      routePath: state.config.routePath,
      token: next.token,
      defaultWorkspace: next.defaultWorkspace,
      repoWorkspaces: next.repoWorkspaces,
      promptTemplate: next.promptTemplate,
      model: next.model,
      poll: { repos: next.repos, intervalMs: next.intervalMs, backfillMs: next.backfillMs },
    }
    try {
      await persistConfigOf(nextConfig)
    } catch (error) {
      console.error('[issue-bot] config persist failed, keeping previous config: ' + String(error))
      return { ok: false, error: '落盘失败,配置未更改:' + String(error && error.message ? error.message : error) }
    }
  ...
  async function handler(req, res) {
    ...
    if (req.method === 'GET' || req.method === 'HEAD') {
      ...
      if (path === base + '/config') {
        ...
        send(res, 200, { ok: true, config: state.config, poll: pollSummary(), modelOptions: await modelCatalog() })
        return
      }
  ...
  // 路由与轮询都在配置加载完成后启动:任何请求都不会看到种子默认值,
  // 重启后打开面板/设置页不会被"空配置"闪现误导
